...
// Data Fields
    ...
    private NonConflictingSchedules.OfferedClass[] parent;
    /** Flag to indicate whether this vertex has been visited. */
    private boolean[] visited;
    /** The array that contains each vertex in discovery order. */
    private LinkedList<NonConflictingSchedules.OfferedClass> discoveryOrder;
    /** The array that contains each vertex in finish order. */
    private NonConflictingSchedules.OfferedClass[] finishOrder;
...
// Constructors /*
    ...
    public DepthFirstSearch(Graph graph) {
        this.graph = graph;
        int n = graph.getNumV();
        parent = new NonConflictingSchedules.OfferedClass[n];
        visited = new boolean[n];
        discoveryOrder = new NonConflictingSchedules.OfferedClass[n];
        finishOrder = new NonConflictingSchedules.OfferedClass[n];
        for (int i = 0; i < n; i++) {
            parent[i] = null;
        }
        for (int i = 0; i < n; i++) {
...
// Same as constructor above except for the if statement.
    ...
    public void depthFirstSearch(int current) {
        ...
        discoveryOrder.add(parent[current]);
        /* Examine each vertex adjacent to the current vertex *//*
        Iterator<Edge> itr = graph.edgeIterator(current); //should be a class
        while (itr.hasNext()) {
