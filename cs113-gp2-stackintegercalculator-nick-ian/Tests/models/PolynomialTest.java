...
public class PolynomialTest {
    ...
    @Test
    public void testDivision() throws PostfixEvaluator.SyntaxErrorException{
        Polynomial poly1 = new Polynomial("-17");
        Polynomial poly2 = new Polynomial("+0");
        //test divide by 0
        try {
            poly1.divide(poly2);
            fail("Division should throw an Arithmetic exception");
        } catch (ArithmeticException e) { /* Test Passed! */ }

        //test negative / positive
        poly1 = new Polynomial("-17");
        poly2 = new Polynomial("+5");
        Polynomial poly3  = poly1.divide(poly2);
        assertEquals("negative/positive is not working", poly3.toString(), "-3");

        //test positive / negative
        poly1 = new Polynomial("+16");
        poly2 = new Polynomial("-3");
        poly3  = poly1.divide(poly2);
        assertEquals("positive/negative is not working", poly3.toString(), "-5");

        //test postive / positive
        poly1 = new Polynomial("+16");
        poly2 = new Polynomial("+4");
        poly3  = poly1.divide(poly2);
        assertEquals("positive/positive is not working", poly3.toString(), "+4");
        ...
        //test rejection of variable division
        poly2 = new Polynomial("+5x");
        try {
            poly1.divide(poly2);
            fail("Division should throw an Arithmetic exception");
        } catch (ArithmeticException e) { /* Test Passed! */ }
    ...
    @Test
    public void testNumberOfTerms() throws PostfixEvaluator.SyntaxErrorException{
        Polynomial poly3;
        //test that the calculator can handle up to 50 terms
        String bigPoly= new String("");
        for(int i = 0; i < 50; i++){
            bigPoly= bigPoly + ("+x^" + i);
        }
        poly3 = new Polynomial(bigPoly);
        assertEquals("The polynomial can not handle at least 50 terms", poly3.getNumTerms(), 50);
    }

    @Test
    public void testMultiplication() throws PostfixEvaluator.SyntaxErrorException{
        Polynomial poly1 = new Polynomial("-17");
        Polynomial poly2 = new Polynomial("+0");

        //test negative * positive for basic terms of the same size
        poly1 = new Polynomial("-17");
        poly2 = new Polynomial("+5");
        Polynomial poly3  = poly1.multiply(poly2);
        assertEquals("negative * positive for basic terms of the same size is not working", poly3.toString(), "-85");

        //test positive * positive for a larger first term
        poly1 = new Polynomial("+16x^2+4");
        poly2 = new Polynomial("+4");
        poly3  = poly1.multiply(poly2);
        assertEquals("postive * positive for a larger first term is not working", poly3.toString(), "+64x^2+16");

        //test negative * negative for a smaller first term
        poly1 = new Polynomial("-4");
        poly2 = new Polynomial("-16x^2-4");
        poly3  = poly1.multiply(poly2);
        assertEquals("negative * negative for a smaller first term is not working", poly3.toString(), "+64x^2+16");

        //test multiplication of variables with matching and non-matching exponents
        poly1 = new Polynomial("+4x^3-2x+6");
        poly2 = new Polynomial("+5x^6+12x-3+17x^8");
        poly3  = poly1.multiply(poly2);
        assertEquals("multiplication of variables with matching and non-matching exponents is not working", poly3.toString(), "+68x^11-14x^9+102x^8-10x^7+30x^6+48x^4-12x^3-24x^2+78x-18");
    }

    @Test
    public void testSubtraction() throws PostfixEvaluator.SyntaxErrorException{
        Polynomial poly1 = new Polynomial("-17");
        Polynomial poly2 = new Polynomial("+0");
        ...
        poly1 = new Polynomial("-17");
        poly2 = new Polynomial("+5");
        Polynomial poly3  = poly1.subtract(poly2);
        assertEquals("negative - positive for basic terms of the same size is not working", poly3.toString(), "-22");

        //test positive - positive for a larger first term
        poly1 = new Polynomial("+16x^2+4");
        poly2 = new Polynomial("+4");
        poly3  = poly1.subtract(poly2);
        assertEquals("postive - positive for a larger first term is not working", poly3.toString(), "+16x^2");

        //test negative - negative for a smaller first term
        poly1 = new Polynomial("-4");
        poly2 = new Polynomial("-16x^2-4");
        poly3  = poly1.subtract(poly2);
        assertEquals("negative - negative for a smaller first term is not working", poly3.toString(), "+16x^2");

        //test multiplication of variables with matching and non-matching exponents
        poly1 = new Polynomial("+4x^3-2x+6");
        poly2 = new Polynomial("+5x^6+12x-3+17x^8");
        poly3  = poly1.subtract(poly2);
        assertEquals("multiplication of variables with matching and non-matching exponents is not working", poly3.toString(), "-17x^8-5x^6+4x^3-14x+9");
    }
}